...
export default function (app, db) {
  ...
  app.post('/api/v1/leaf_spectra/csv/', function (req, res) {
    if(typeof req.body.ids !== 'undefined' && req.body.type=='mean'){
      ...
      db.query("SELECT s.sample_id, l.site_id, l.scientific_name, s.leaf_number, l.date_measured, s.leaf_side_measured, wavelength, reflectance_transmittance, calculated_value FROM spectra_leaves s LEFT JOIN leaf_spectra l ON(s.sample_id::text=l.sample_id::text) WHERE s.sample_id IN("+req.body.ids+") ORDER BY sample_id, leaf_number, wavelength;", { type: db.QueryTypes.SELECT }).then(result => {
        try {
          const parser = new Parser();
          const csv = parser.parse(result);
          res.status(200).send(csv);
