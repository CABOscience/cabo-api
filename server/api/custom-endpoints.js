...
export default function (app, db) {
  ...
  app.post('/api/v1/leaf_spectra/search/taxa', function (req, res) {
      let select = 'SELECT sample_id, b.scientific_name, o.permission FROM plants p LEFT JOIN bulk_leaf_samples b ON (p.fulcrum_id=b.plant OR p.fulcrum_id=b.plant2) LEFT JOIN projects_permissions o ON p.project=o.project';
  ...
  app.post('/api/v1/leaf_spectra/csv/', function (req, res) {
    if(typeof req.body.ids !== 'undefined'){
      ...
      if(req.body.type=='mean'){
        ...
        db.query("SELECT s.sample_id, l.site_id, l.scientific_name, s.leaf_number, l.date_measured, s.leaf_side_measured, wavelength, reflectance_transmittance, calculated_value FROM spectra_leaves s LEFT JOIN leaf_spectra l ON(s.sample_id2=l.sample_id) WHERE l.sample_id IN("+ids+") ORDER BY sample_id, leaf_number, wavelength;", { type: db.QueryTypes.SELECT }).then(result => {
          try {
            const parser = new Parser();
            const csv = parser.parse(result);
            res.status(200).send(csv);
  ...
  app.post('/api/v1/traits/csv/', function (req, res) {
    if(typeof req.body.ids !== 'undefined' && req.body.type=='raw'){
      ...
      db.query("SELECT * FROM all_traits WHERE sample_id IN("+ids+");", { type: db.QueryTypes.SELECT }).then(result => {
        try {
          const parser = new Parser();
          const csv = parser.parse(result);
          res.status(200).send(csv);
